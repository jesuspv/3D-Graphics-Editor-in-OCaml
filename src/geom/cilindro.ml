...
class cilindro
   ...
   method dibujar ~normales ~cara ?(sombra = false) () =
      ...
      GluQuadric.cylinder ~base:base ~top:base ~height:altura ~slices:meridianos ~stacks:paralelos ~quad ();
      GlMat.pop ();
      (* tapa superior *)
      GlDraw.begins `triangle_fan;
         GlDraw.normal3 (0.0, 1.0, 0.0);
         GlDraw.vertex3 (0.0, altura /. 2.0, 0.0);
         for i = 0 to meridianos do
